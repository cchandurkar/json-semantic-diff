...
interface FlatNode { path: string; label: string; changeKind: string; left?: JsonValue; right?: JsonValue; }
...
describe('diffJson - golden example snapshot', () => {
  const result = diffJson(
    JSON.parse(EXAMPLE_LEFT) as JsonValue,
    JSON.parse(EXAMPLE_RIGHT) as JsonValue,
